#!/usr/bin/python3

# Функция ввода числа с проверкой
def Num(txt):
    while True:
        _ = input(f'{txt}: ')
        if _.isnumeric(): return int(_)
        else: print('Ошибка: Введенные данные не содержат чило!\n')

val = [0, 0, 0]

# Задание 1
print('ЗАДАНИЕ1\nПользователь вводит с клавиатуры три числа. В зависимости от выбора пользователя программа выводит на экран сумму трёх чисел или произведение трёх чисел.\n\nРЕШЕНИЕ:')

for _ in range(len(val)):
    val[_] = Num(f'Ввведите число {_ + 1}')
print('\nДействие:\n\t1: сумма трех чисел\n\t2: произведение трех чисел')
while True:
    go = Num('Введите нужное действие')
    match go:
       case 1: print(f'\nСумма трех чисел: {val[0] + val[1] + val[2]}')
       case 2: print(f'\nПроизведение трех чисел: {val[0] * val[1] * val[2]}')
       case _: print(f'Ошибка! Вы ввели неправильное действие!\n'); continue
    break

# Задание 2
print('\n\nЗАДАНИЕ2\nПользователь вводит с клавиатуры три числа. В зависимости от выбора пользователя программа выводит на экран максимум из трёх, минимум из трёх или среднеарифметическое трёх чисел\n\nРЕШЕНИЕ:')
for _ in range(len(val)): val[_] = Num(f'Ввведите число {_ + 1}')
print('\n\nДействие:\n\t1: максимум из трех чисел\n\t2: минимум из трех чисел\n\t3: среднеарифметическое трех чисел')
while True:
    go = Num('Введите нужное действие')
    match go:
       case 1: print(f'\nМаксимальное из трех чисел: {max(val)}')
       case 2: print(f'\nМинимальное из трех чисел: {min(val)}')
       case 3: print(f'\nСреднеарифметическое из трех чисел: {sum(val) / len(val)}')
       case _: print(f'Ошибка! Вы ввели неправильное действие!\n'); continue
    break

# Задание 3
print('\n\nЗАДАНИЕ3\nПользователь вводит с клавиатуры количество метров. В зависимости от выбора пользователя программа переводит метры в мили, дюймы или ярды\n\nРЕШЕНИЕ:')
val[0] = Num('Введите колличество метров')
print(f'\nПеревести {val[0]}м. в:\n\t1: мили\n\t2: дюймы\n\t3: ярды')
while True:
    go = Num('Введите нужное действие')
    match go:
       case 1: print(f'\n{val[0]}м. в милях: {val[0] * 0.000621}')
       case 2: print(f'\n{val[0]}м. в дюймах: {val[0] * 39.37}')
       case 3: print(f'\n{val[0]}м. в ярдах: {val[0] * 1.09}')
       case _: print(f'Ошибка! Вы ввели неправильное действие!\n'); continue
    break
