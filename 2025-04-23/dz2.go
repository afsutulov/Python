...
func main() {
    // Задание 1
    fmt.Println("ЗАДАНИЕ1: Пользователь вводит с клавиатуры строку. Проверьте является ли введенная строка палиндромом. Палинслева направо и справа налево.\n\nРЕШЕНИЕ:")
    txt := strings.ReplaceAll(strings.ToLower(Num("Введите строку")), " ", "")
    if txt == Reverse(txt) { fmt.Println("РЕЗУЛЬТАТ: Слово является палиндромо") } else { fmt.Println("РЕЗУЛЬТАТ: Слово неявляется палиндромо") }
