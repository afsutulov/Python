...
class Game:
    def __init__(self, root):
        ...
        self.exit_button = tk.Button(self.button_frame, text="Выход", command=root.quit, relief="flat", fg=self.clr[2], bg=self.clr[3], activeforeground=self.clr[1], activebackground=self.clr[2], width=20)
    ...
    def Hod(self, x, y):
        ...
        while Flag:
            ...
            match self.p1[x][y]:
                case "#":
                    if self.p1[x - 1][y] != "-": self.hod.append([x - 1, y]) # Проверяем соседа на возможность хода. Если ок - добавляем в список ходов ИИ
                    if self.p1[x + 1][y] != "-": self.hod.append([x + 1, y]) # Проверяем соседа на возможность хода. Если ок - добавляем в список ходов ИИ
                    if self.p1[x][y - 1] != "-": self.hod.append([x, y - 1]) # Проверяем соседа на возможность хода. Если ок - добавляем в список ходов ИИ
                    if self.p1[x][y + 1] != "-": self.hod.append([x, y + 1]) # Проверяем соседа на возможность хода. Если ок - добавляем в список ходов ИИ
                    for i in range(x - 1, x + 2): # Заполняем область вокруг исключениям, чтобы повторно не стрелять (только если точка в списке ходов ИИ)
                        for j in range(y - 1, y + 2):
                            if self.p1[i][j] == " ": self.p1[i][j] = "-"
